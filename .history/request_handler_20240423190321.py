...
class HandleRequests(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        (resource, id_or_query) = parsed

        if resource == "animals":
            if id_or_query is not None:
                response = get_single_animal(id_or_query)
            else:
                response = get_all_animals()
        elif resource == "locations":
            if id_or_query is not None:
                response = get_single_location(id_or_query)
            else:
                response = get_all_locations()
        elif resource == "employees":
            if id_or_query is not None:
                response = get_single_employee(id_or_query)
            else:
                response = get_all_employees()
        elif resource == "customers":
            if id_or_query is not None:
                response = get_single_customer(id_or_query)
            else:
    ...
    def do_POST(self):
        ...
        # Create new entity based on the resource type
        if resource == "animals":
